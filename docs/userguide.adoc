...
```bash
$ which fopub
~/path/to/fopub: You must specify a DocBook v4.5 or DocBook v5 XML source file as the first command argument
```
...
Documents with two Level 0 (`=`) Sections need the `:doctype: book` sttribute set.

It's illegal to skip section like so:
...
TIP: This is convenient, but, of course, no positional attributes i.e. `[blocktype,attr1,attr2]` can be specified.

Nesting blocks of the same type is done using an extra delimiter on the containing block like the following:
...
It's also possible using the `role` attribute:
...
When syntax highlighting using http://prismjs.com/[Prism.js], CSS classes can be set inline and the language of inline source code can be set as follows:

.Set Syntax Highlighting Language Inline
[source,asciidoc]
...
*Output:* The favicon is located [language-html]`<a href="icons/favicon.ico"/>` in your header. +
The next thing was the menubar: `<div id="menubar">` which contained a list `<ul class="fa-ul">`...


Instead of writing [language-adoc]`[language-html]` before every piece of inline code I want highlighted, I can set a CSS class to a paragraph. All inline source code within that paragraph will then be highlighted with the language specified:
...
Instead of setting a CSS class on every paragraph, it's also possible to set a class on a section.
...
TIP: The highest section level that a CSS class can be applied on is the Level 1 (`==`) and all contained sections will inherit this class.
