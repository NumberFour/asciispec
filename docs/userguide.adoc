...
Another common CLI option is `-D` which is to select a destination directory.
Assuming a folder `output` exists in the current directory, you would use the following command to generate `userguide.html` there:

`asciispec -D output userguide.adoc`
...
The default way of adding CSS classes to blocks in Asciidoc is with the syntax `[.css-class]` on the preceding line or by using the `role=` attribute:
...
It's also possible to apply CSS classes to code blocks. This is useful for changing the text size for long console logs, for instance:
Let's set the CSS class `xx-small` on the source block below using the following line:
...
- [x] All these list items will be red, too!ed
--
