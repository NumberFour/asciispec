...
[source,asciidoc]
The favicon is located [language-html]`<a href="favicon.ico"/>` in your header.
...
The line below checks if there is an attribute `revnumber` and if it exists, include the content in the square brackets:
...
This can also be done using larger blocks of content as follows:
