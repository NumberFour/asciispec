...
public class SimpleParser {
	...
	protected static class Token<TokenType> extends TokenPosition {
		...
		public final Token<TokenType> expect(TokenType... expected) throws ParseException {
			if (hasType(expected))
				return this;
	...
	protected static abstract class SimpleTokenizer<TokenType> {
		 ...
		 */
		public Token<TokenType> nextToken() throws ParseException {
		 ...
		 *             if an error occurs
		 */
		protected abstract Token<TokenType> readToken() throws ParseException;
		 ...
		 */
		protected String readDelimitedString(char delimiter, char escapeChar) throws ParseException {
			...
			throw new ParseException(line, column, "Expected '" + delimiter + "', but reached end of file");
		}
		...
		/**
		 * Forwards the current parsing position until the start of the given sequence or the position until the end of
		 * the file.
		 *
		 * @param sequence
		 *            the string sequence to stop after
		 * @return the number of character read
		 */
		protected int readUntilSequence(final String sequence) {
			if (sequence.isEmpty())
				return 0;

			char firstDelimiter = sequence.charAt(0);

			while (!eof()) {
				int count = readUntil(String.valueOf(firstDelimiter));

				String nextSeq = getExcerpt(getCurrentPosition(), sequence.length());
				if (sequence.equals(nextSeq))
					return count;
		 ...
		 */
		protected void throwIfEof() throws ParseException {
			if (eof())
				throw new ParseException(line, column, "Unexpected end of file");
		}
